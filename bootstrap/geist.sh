...
login() {
    ...
    for user in $@; do
        ...
        sshpass -p "$DEFAULT_PASSWORD" ssh -o "StrictHostKeyChecking no" $user@$ip "bash -s" < $SCRIPT >/dev/null 2>&1
        if [ $? -eq 0 ]; then
...
while [[ "$#" -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "  -s, --script SCRIPT          Script to execute on the remote hosts"
            ;;
